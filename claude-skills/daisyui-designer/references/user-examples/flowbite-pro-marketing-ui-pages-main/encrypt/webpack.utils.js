...
const transformEncryptedJs = ({ fileName, classNames = {}, overwrite = true }) => {
	...
	Object.keys(classNames).forEach(className => {
		...
		const regex2 = new RegExp(`(dragClass|ghostClass): '(${escapedClassName})',`)
		// regex used to replace the classes added by flowbite library js
		const regex3 = new RegExp(`(activeClasses|inactiveClasses|backdropClasses\:)(.+)(([ |'])${escapedClassName}([ |']))(.+)`)
		// find dynamic classes for drawers
		const regex4 = new RegExp(`(base|active|inactive\:)(.+)(([ |'])${escapedClassName}([ |']))(.+)`)
		// regex used to replace classes related to tippy tooltip 
		// const regex4 = new RegExp(`(BOX_CLASS|CONTENT_CLASS|BACKDROP_CLASS|ARROW_CLASS|SVG_ARROW_CLASS) = "(${className})";`)
		...
		jsContent = jsContent
			...
			.replace(regex3, (match, p1, p2, p3, p4, p5, p6) => (
				`${p1}${p2}${p4}${encryptedClassName}${p5}${p6}`
			))
			.replace(regex4, (match, p1, p2, p3, p4, p5, p6) => (
				`${p1}${p2}${p4}${encryptedClassName}${p5}${p6}`
			));
	});
